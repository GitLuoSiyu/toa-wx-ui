export default Behavior({
    ...
    methods: {
        ...
        getLatestTime() {
            ...
            if ("normal" === n) {
                if ("second" !== i && (s = "string" == typeof t ? s.replace(/-/g, "/") : s, s = Math.ceil((new Date(s).getTime() - (new Date).getTime()) / 1e3)), s < 0 && "second" !== i) return this._getTimeValue(0), void this.CountdownEnd();
                s - a > 0 ? this.getLatestForCountDown(s) : s - a < 0 ? this.getLatestForAddTime(s) : s - a == 0 && (a <= 0 && this._getTimeValue(s), this.CountdownEnd()), e && s - a != 0 && this.init.call(this)
            } else "anniversary" === n ? "second" === i ? console.error(`倒计时为${n}类型时，不可设置timeType值为second`) : (s = "string" == typeof t ? s.replace(/-/g, "/") : s, s = Math.ceil(((new Date).getTime() - new Date(s).getTime()) / 1e3), s >= 0 ? (this.getLatestForCountDown(s), this.init.call(this)) : console.error("time传值错误")) : console.error("countdownType类型错误")
        },
